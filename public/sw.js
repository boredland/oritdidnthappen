...
// Background Fetch (Chromium/Android): the browser keeps uploading a photo
// batch even after the PWA is closed. Each request is persisted server-side on
// receipt, so we don't replay responses — we only surface completion. On
// success, ping any open client so its gallery refreshes; if none is open and
// some uploads failed, tell the user.
self.addEventListener("backgroundfetchsuccess", function (event) {
  event.waitUntil(
    (async function () {
      ...
      for (var i = 0; i < clients.length; i++) {
        clients[i].postMessage({ type: "bgupload-done", id: event.registration.id });
      }
    })(),
  );
});
...
self.addEventListener("backgroundfetchfail", function (event) {
  event.waitUntil(
    (async function () {
      try {
        await event.updateUI({ title: "Some photos didn't upload" });
      } catch (_) {}
      var clients = await self.clients.matchAll({
        type: "window",
        includeUncontrolled: true,
      ...
      if (clients.length === 0) {
        await self.registration.showNotification("Some photos didn't upload", {
          body: "Open the event and try again.",
          icon: "/icon-192.png",
          badge: "/icon-192.png",
          tag: "bgupload-fail",
        });
...
// Tapping the browser's background-fetch UI focuses (or opens) the app.
